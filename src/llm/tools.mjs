 ...
 * Import and call this before starting a coordination cycle.
 */
...
async function sendMessage(input) {
  ...
  try {
    const { recipientId, message } = typeof input === "string" ? JSON.parse(input) : input;
...
async function coordinatePickup(input) {
  ...
  try {
    ...
    const reply = await socket.emitAsk(teammateId, { action: "pickup", parcelId });
