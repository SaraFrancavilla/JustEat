...
// Array of deceptive/manipulative messages designed to confuse other agents
...
// Start the loop (runs every 60 seconds)
setInterval(() => {
    // Only chat if the game is active and we are connected
    if (W && W.me && W.me.id) {
        // Pick a random message
        const randomIndex = Math.floor(Math.random() * EVIL_MESSAGES.length);
        const fakeMessage = EVIL_MESSAGES[randomIndex];
        
        // Broadcast to all agents using the 'shout' method
